...
class FrameStackMLP(nn.Module):
    ...
    Input format (from MeleeFrameDataset):
        float_ctx: (B, K, 40) — continuous + binary + controller per frame
        int_ctx:   (B, K, 4)  — [p0_action, p0_jumps, p1_action, p1_jumps]
    """
    ...
    def __init__(
        ...
        # Per-frame dim: 40 (floats) + 2*(action_embed + jumps_embed) = 40 + 72 = 112
        embed_per_frame = 2 * (cfg.action_embed_dim + cfg.jumps_embed_dim)
        frame_dim = 40 + embed_per_frame
        input_dim = frame_dim * context_len
    ...
    def forward(
        ...
        Args:
            float_ctx: (B, K, 40) — float features per frame
            int_ctx:   (B, K, 4)  — [p0_action, p0_jumps, p1_action, p1_jumps]
        ...
        p1_action_emb = self.action_embed(int_ctx[:, :, 2])  # (B, K, 32)
        p1_jumps_emb = self.jumps_embed(int_ctx[:, :, 3])  # (B, K, 4)

        # Concat floats + embeddings per frame: (B, K, frame_dim)
        frame_enc = torch.cat([
            float_ctx,
            p0_action_emb, p0_jumps_emb,
            p1_action_emb, p1_jumps_emb,
        ], dim=-1)
