#!/usr/bin/env python3
"""
Run Phillip vs SmashBot.

Usage (from project root):
    .venv/bin/python scripts/run_phillip_vs_smashbot.py
"""
...
SMASHBOT_PATH = PROJECT_ROOT / 'fighters/smashbot/SmashBot'
...
print(f"  SmashBot: {SMASHBOT_PATH}")
...
if not SMASHBOT_PATH.exists():
    print(f"\n❌ SmashBot not found at {SMASHBOT_PATH}")
    print("See fighters/smashbot/README.md for setup")
    sys.exit(1)
...
def main():
    ...
    try:
        ...
        print(f"  ✅ Phillip: {phillip.name()}")
        print(f"     {phillip.description()}")
    ...
    try:
        smashbot = SmashBotFighter(str(SMASHBOT_PATH))
        print(f"  ✅ SmashBot: Ready")
    except Exception as e:
        print(f"  ❌ Failed to create SmashBot: {e}")
        import traceback
        traceback.print_exc()
        return 1
    ...
    print(f"  P2 (SmashBot): {settings.p2_character.name}")
    ...
    # Run the match!
    print("\n" + "=" * 60)
    print("STARTING MATCH: Phillip vs SmashBot")
    print("=" * 60)
    print("\nDolphin should launch... Press Ctrl+C to stop\n")

    try:
        def on_game_end(game):
            print(f"\n🏁 Game {game.game_num} complete!")
